...
class CarGame:
    ...
    def collect_data(self):
        ...
        self.data.append({
            'time': time.time(),
            'input_north': self.counters['input_north'],
            'input_south': self.counters['input_south'],
            'input_east': self.counters['input_east'],
            'input_west': self.counters['input_west']
        })

    def update_plots(self, ax):
        ax.clear()
        times = [d['time'] for d in self.data]
        north_counts = [d['input_north'] for d in self.data]
        south_counts = [d['input_south'] for d in self.data]
        east_counts = [d['input_east'] for d in self.data]
        west_counts = [d['input_west'] for d in self.data]
        ax.plot(times, north_counts, label='North')
        ax.plot(times, south_counts, label='South')
        ax.plot(times, east_counts, label='East')
        ax.plot(times, west_counts, label='West')
        ax.legend()
        plt.draw()
        plt.pause(0.01)
