...
class CarGame:
    ...
    def update_model(self):
        ...
        reward =  - self.calculate_waiting_time()
    ...
    #         # Check if a full generation has passed (all signals have been green once)
    #         if self.current_signal_index == 0:
    #             self.generation += 1  # Increment generation counter
    #             self.update_model()  # Update the model after each generation
    #             self.collect_data()  # Collect data for the current generation
    ...
    #   ax.plot(generations, cars_passed, label='Cars Passed per Generation', color='blue')
    #   ax.plot(generations, rewards, label='Reward per Generation', color='yellow')
